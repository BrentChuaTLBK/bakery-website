...
Backend deployed: migration `newsletter_welcome_discount` (remote version `20260923141444`, source `20260923140252_newsletter_welcome_discount.sql`) and email-worker v13. The newsletter endpoint remains v9. Deploy the email renderer before the SQL migration on another environment, then publish the changed static files together. No existing subscribers were emailed. A live signup/issuance/report verification ran inside a transaction and rolled back its fixtures; the six pre-existing subscriber records remained ineligible. Real inbox delivery was not exercised. Security advisories were unchanged.
