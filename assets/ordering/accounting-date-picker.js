...
function calendarView(picker, focusDate='') {
 ...
 const jump=`<div class="accounting-calendar-jump">${mode==='date'?`<label>Month<select data-date-month>${MONTHS.map((name,i)=>`<option value="${pad(i+1)}" ${month.slice(5)===pad(i+1)?'selected':''}>${name}</option>`).join('')}</select></label>`:''}<label>Year<input type="number" data-date-year value="${year}" min="1" max="9999" step="1" inputmode="numeric" aria-label="Calendar year"></label></div>`;
 let grid;
 if(mode==='month') grid=`<div class="accounting-month-grid">${MONTHS.map((name,i)=>{const value=year+'-'+pad(i+1);return `<button type="button" class="calendar-day" data-date-value="${value}" aria-label="${name} ${year}" aria-pressed="${selected===value}" ${today.startsWith(value)?'aria-current="date"':''}>${name.slice(0,3)}</button>`;}).join('')}</div>`;
 else {
  ...
  const rows=[];
  for(let i=0;i<cells.length;i+=7)rows.push(`<tr>${cells.slice(i,i+7).map(date=>date?`<td><button type="button" class="calendar-day" data-date-value="${date}" aria-label="${esc(label(date,'date'))}" aria-pressed="${selected===date}" ${today===date?'aria-current="date"':''} tabindex="${date===tab?'0':'-1'}">${Number(date.slice(8))}</button></td>`:'<td></td>').join('')}</tr>`);
 ...
 return toolbar+jump+grid+`<div class="calendar-footer"><button type="button" class="calendar-today" data-date-today>Current ${mode==='month'?'year':'month'}</button><button type="button" class="calendar-today" data-date-close>Close calendar</button></div>`;
}

export function accountingDatePicker(name,title,value,today,{mode='date'}={}) {
 if(!valid(value,mode)||!isCalendarDate(today))throw Error('Choose a valid calendar date.');
 const id='accounting-date-'+(++sequence);
 return `<div class="field accounting-date-field"><span id="${id}-label">${esc(title)}</span><details class="accounting-date-picker" data-mode="${mode}" data-today="${today}" data-month="${value.slice(0,7)}"><summary aria-labelledby="${id}-label ${id}-value"><span id="${id}-value" data-date-label>${label(value,mode)}</span><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="5" width="18" height="16" rx="3"/><path d="M7 3v4m10-4v4M3 11h18"/></svg></summary><input type="hidden" name="${esc(name)}" value="${value}"><div class="date-calendar accounting-calendar" aria-labelledby="${id}-label"></div></details></div>`;
}
...
export function setAccountingDate(root,name,value,{notify=false}={}) {
 ...
 if(!valid(value,picker.dataset.mode))throw Error('Choose a valid calendar date.');
 input.value=value;picker.dataset.month=value.slice(0,7);picker.querySelector('[data-date-label]').textContent=label(value,picker.dataset.mode);
 render(picker);
...
export function bindAccountingDates(root) {
 ...
 root.addEventListener('toggle',event=>{
  const picker=event.target;if(!picker.matches?.('.accounting-date-picker')||!picker.open)return;
 ...
 root.addEventListener('change',event=>{
  ...
  if(!/^[0-9]{1,4}$/.test(year)||Number(year)<1||!isCalendarDate(value+'-01')){input.setCustomValidity('Enter a year from 1 to 9999.');input.reportValidity();return;}
  input.setCustomValidity('');picker.dataset.month=value;render(picker,'',input.hasAttribute('data-date-year')?'[data-date-year]':'[data-date-month]');
 });
 root.addEventListener('keydown',event=>{
  ...
  const next=calendarKeyDate(button.dataset.dateValue,event.key,event.shiftKey);if(!next||!isCalendarDate(next)||next.startsWith('0000'))return;
  event.preventDefault();picker.dataset.month=next.slice(0,7);render(picker,next);
 });
 if(!outsideBound){outsideBound=true;document.addEventListener('click',event=>document.querySelectorAll('.accounting-date-picker[open]').forEach(picker=>{if(!picker.contains(event.target))picker.open=false;}));}
}
