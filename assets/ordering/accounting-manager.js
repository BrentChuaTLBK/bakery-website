  ...
  root.addEventListener('submit',async event=>{
    ...
    try {
      ...
      if(form.classList.contains('accounting-filters')) {
        ...
        await api('accounting_save_entry',{id:draft.id,revision:draft.revision,entry_date:f.get('entry_date'),category_id:category,amount_cents:parseAccountingAmount(f.get('amount')),note:f.get('note')});
  ...
  root.addEventListener('click',async event=>{
    ...
    try{
      ...
      if(action==='history'){
        ...
        $('.accounting-history').innerHTML=`<h3>Entry history</h3><ol class="history">${rows.map(r=>`<li>${esc(new Date(r.at).toLocaleString('en-PH',{timeZone:'Asia/Manila'}))} · ${r.action==='accounting_delete_entry'?'Removed':'Saved'}<p>${esc(r.after.entry_date)} · ${money(r.after.amount_cents)} · ${esc(r.after.note)}</p></li>`).join('')}</ol>`;
      }
