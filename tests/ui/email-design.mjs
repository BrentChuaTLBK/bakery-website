...
const report=[],files=(await readdir(folder)).filter(f=>f.endsWith('.html')&&f!=='index.html');
for(const width of [900,390,320]){
 ...
 for(const file of files){
  ...
  if(['out_for_delivery_tracking.html','delivery_tracking_updated.html'].includes(file)){const button=page.getByRole('link',{name:'Track delivery',exact:true});assert.equal(await button.count(),1);assert.match(await button.getAttribute('href'),/^https:\/\/www\.(?:lalamove|grab)\.com\//);}
  if(file==='delivery_tracking_unavailable.html'){assert.equal(await page.getByRole('link',{name:'Track delivery',exact:true}).count(),0);assert.match(await page.locator('body').innerText(),/Courier tracking is temporarily unavailable/);}
  if(['ready_for_pickup.html','newsletter_offer.html','account_confirmation.html','out_for_delivery_tracking.html','delivery_tracking_updated.html','delivery_tracking_unavailable.html'].includes(file)){
...
await writeFile(join(folder,'browser-checks.json'),JSON.stringify(report,null,2));console.log(`PASS ${files.length} templates at 900/390/320px; images blocked; headings, alt text, tracking and links.`);console.log(JSON.stringify(report));await browser.close();
