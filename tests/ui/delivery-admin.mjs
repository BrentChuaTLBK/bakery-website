...
try {
  ...
  console.log('PASS: independent calendar navigation, keyboard selection, saved date removal/persistence, unsaved form preservation, mobile width, owner permissions, product pickup-only editing, and escaped multiline zone descriptions. Local API only.');
} finally {
  await browser?.close();
