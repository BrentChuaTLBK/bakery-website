...
const product = { id: 'calendar-nori', name: 'Calendar nori fixture', description: 'Local test only.', price_cents: 13000, min_quantity: 1, lead_days: 0, active: true, photos: [], option_groups: [] };
const inventory = [];
for (let date = new Date('2026-09-16T12:00:00Z'); date <= new Date('2026-11-30T12:00:00Z'); date.setUTCDate(date.getUTCDate() + 1)) {
  inventory.push({ product_id: product.id, date: date.toISOString().slice(0, 10), capacity: 100, remaining: 100, available: true });
}
const catalog = {
  products: [product], categories: [], inventory,
  zones: [{ id: 'local-zone', name: 'Local delivery area', localities: ['Fixture City'], active: true, fee_cents: 10000 }],
  settings: { paused: false, shop_name: 'Calendar fixture', production_weekdays: [0, 1, 2, 3, 4, 5, 6], fulfillment_weekdays: [0, 1, 2, 3, 4, 5, 6], nonproduction_dates: ['2026-09-21'], blocked_dates: ['2026-09-18'], delivery_blocked_dates: ['2026-09-19'], pickup_blocked_dates: ['2026-09-20'], pickup_address: 'Local test kitchen', pickup_hours: '10 AM onwards', pickup_instructions: 'Local test only.', payment_instructions: 'Do not send payment.' },
};
...
export async function api(action, payload = {}) {
  ...
  if (action === 'quote') {
    const items = payload.items.map(item => ({ ...item, name: catalog.products[0].name, unit_price_cents: 13000, line_total_cents: 13000 * item.quantity, selection_labels: [] }));
...
try {
  ...
  assert.equal(await day('2026-09-15').isDisabled(), true, 'Same-day booking stays unavailable');
  ...
  for (const key of ['ArrowLeft', 'PageUp', 'Shift+PageUp']) {
    ...
    await page.keyboard.press(key);
    assert.equal(await month.textContent(), 'September 2026');
    assert.equal(await page.evaluate(() => document.activeElement?.dataset.customerDate), '2026-09-16', key + ' cannot focus a past or same-day date');
  }
  ...
  console.log('PASS: calendar past/same-day and month bounds, Philippine timezone, method closures, keyboard bounds, clear/reopen, basket and buyer persistence, review date, stale dates, mobile fit, no API mutations or external service requests.');
} finally {
  await browser?.close();
